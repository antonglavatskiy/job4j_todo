...
class HQLUserRepositoryTest {
    ...
    public void whenSaveUser() {
        Optional<User> user = userRepository.save(new User(0, "name", "login", "pass"));
        assertThat(user).isNotEmpty();
    }

    @Test
    public void whenSaveUserThenFindByLoginAndPassword() {
        User user = userRepository.save(new User(0, "name", "login", "pass")).get();
    ...
    public void whenSaveUsersThenGetListAll() {
        User user1 = userRepository.save(new User(0, "name1", "login1", "pass1")).get();
        User user2 = userRepository.save(new User(0, "name2", "login2", "pass2")).get();
        User user3 = userRepository.save(new User(0, "name3", "login3", "pass3")).get();
